interface Props {
  onNavigateEstimation: () => void;
  onNavigateSteelBeam: () => void;
}

export default function SystemMenu({ onNavigateEstimation, onNavigateSteelBeam }: Props) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-[#f8f9fa]" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
        ...
        <button
          onClick={onNavigateSteelBeam}
          className="w-full h-12 mt-3 bg-indigo-50 hover:bg-indigo-100 active:bg-indigo-200 text-indigo-700 text-base font-semibold rounded-xl border border-indigo-100 transition-all duration-200 active:scale-[0.98]"
        >
          鋼構簡支梁
        </button>
