...
import { NumberField, SelectField, ResultRow, RebarDetailSection, RebarLine, SectionHeader, SafetyBanner } from '../FormFields';
...
export default function EquipFoundForm({ comp, onUpdate }: Props) {
  ...
  return (
    <div className="space-y-2">
      ...
      {result.requiredWeight > 0 && (
        <div className="mt-3">
          <SafetyBanner
            label="重量檢核"
            ir={result.requiredWeight / Math.max(result.foundWeight, 0.001)}
            capacity={{ label: '基礎重', value: result.foundWeight, unit: 'kg' }}
            demand={{ label: '需求重', value: result.requiredWeight, unit: 'kg' }}
          />
        </div>
      )}
